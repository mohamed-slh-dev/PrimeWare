        ...
        <div class="row">
            <div class="col-12">
                <div class="chat-box-left">
                    ...
                    {{-- customers with newest messages --}}
                        ...
                        <div class="tab-pane fade show active" id="general_chat">
                            ...
                            <a href="javascript:void(0);" id="button-{{ $counter }}" class="media chats-button" style="border-bottom: 1px solid lightgrey">
                                ...
                                <div class="media-left">
                                    ...
                                    <img src="{{ asset('assets/img/customers/avatar.png') }}" alt="user" class="rounded-circle thumb-md">

                                    {{-- online status --}}
                                    @if ($customer->onlinestatus == "online")
                                ...
                                <div class="media-body">
                                    <div class="d-inline-block">
                                        <h6>{{ $customer->name }}</h6>
                                        <p>{{ (!empty($messages->where('customer_id', $customer['id'])->last()) ? '["'. $messages->where('customer_id', $customer['id'])->last()->message .'"]' : "Chat Now!" ) }}</p>
                                    </div>
                                    <div>
                                        <span style="font-size:8px;">{{ (!empty($messages->where('customer_id', $customer['id'])->last()) ? $messages->where('customer_id', $customer['id'])->last()->date : "" ) }}</span>

                                        @if ($messages->where('customer_id', $customer['id'])->sum('seen') > 0)
                                        <span></span>
                                        @endif
                    ...
                    @endif
                        ...
                        <div class="chat-body" style="background-color: #364358; box-shadow: 0px 0px 1px 1px #364358">
                            <div class="chat-detail slimscroll">
                                ...
                                @foreach ($messages->where('customer_id', $customer['id']) as $message)
                        ...
                        <div class="chat-footer">
                            ...
                            <form action="{{ route('partner.sendmessagecustomer') }}" method="post">
                                @csrf
                                @method('POST')
